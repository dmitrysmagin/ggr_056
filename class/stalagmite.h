class stalagmite : public _sprite
 {
  private:
   enum { N_BU=5 };
  ...
  public:
   stalagmite(char *,int,int,BITMAP *,int i=0,stalagmite *p=NULL);
   ~stalagmite();
...
stalagmite::stalagmite(char *filename, int sx, int sy, BITMAP *bmp, int i, stalagmite *p) : _sprite(filename,bmp,sx,sy,i,p)
 {
  ...
  ti[2]=new timer(100); // Fire time 1
  ti[3]=new timer(350); // Fire time 2
  ti[4]=new timer(0);
 }
...
void stalagmite::loadSample(char *filename, int n)
 {
